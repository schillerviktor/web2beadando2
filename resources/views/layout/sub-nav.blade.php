@if ((count($menu->children) > 0) AND ($menu->parent_id > 0))
    <li class="nav-item dropdown">
        <a href="{{ url($menu->slug) }}" class="nav-link dropdown-toggle" role="button" data-hover="dropdown">
            {{ $menu->menu_title }}
...
@else
    <li class="nav-item @if($menu->parent_id === 0 && count($menu->children) > 0) dropdown @endif">
        <a href="{{ url($menu->slug) }}" class="nav-link dropdown-toggle" data-hover="dropdown">
            {{ $menu->menu_title }}
            @if(count($menu->children) > 0)
